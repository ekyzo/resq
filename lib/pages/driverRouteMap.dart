...
//THIS IS FOR THE DRIVER MUST ARRIVED OR AT LEAST <1KM FROM DESTINATION TO COMPLETE ORDER
...
class DriverRouteMap extends StatefulWidget {
  ...
  const DriverRouteMap({
    Key? key,
    required this.initialDriverLocation,
    required this.orderId, // Add this line to receive the orderId
  }) : super(key: key);
...
class _DriverRouteMapState extends State<DriverRouteMap> {
  ...
  final String _apiKey =
      'AIzaSyCTG6j7tIvQrts8ZJ0eHQ8dJ4BzwHftxPg'; // Replace with your Google Maps API key
  LatLng _driverLocation = LatLng(0, 0);
  LatLng? _hospitalLocation;
  String? _hospitalName;
  String? _patientName;
  ...
  String? _distance;
  String? _duration;
  bool _isCompleteButtonEnabled = false; // Add this line to track button state
      ...
      if (response.statusCode == 200) {
        ...
        if (data['routes'] != null && data['routes'].isNotEmpty) {
          ...
          setState(() {
            ...
            double distanceValue = double.parse(legs['distance']['value'].toString());
            _isCompleteButtonEnabled = distanceValue < 1000; // Less than 1 km
          });
        } else {
          print('No routes found');
        }
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (_hospitalName != null)
                    Text('Destination: $_hospitalName'),
                  if (_patientName != null) Text('Patient Name: $_patientName'),
                  if (_driverName != null) Text('Driver Name: $_driverName'),
                  SizedBox(height: 10),
                  Center(
                    child: ElevatedButton(
